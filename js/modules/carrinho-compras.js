export default function compras(params) {
  ...
  const btnFecharMobile = document.querySelector(".fechar-mobile p");
  ...
  function clickForaModal(e) {
    if (e.target === this) sectionCompras.classList.remove("ativo");
  }
  ...
  function handleClickEnviar(params) {
    const url = `https://wa.me/${telefone}?text=Gostaria de fazer meu pedido raiz! ${encodeURIComponent(
      mensagem
    )}`;
